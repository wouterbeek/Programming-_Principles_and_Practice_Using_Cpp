...
public:
  ...
  template <class F> Matrix &apply(F f) {
    this->base_apply(f);
    return *this;
  }
  template <class F> Matrix &apply(F f, const T &c) {
    this->base_apply(f, c);
    return *this;
  }
...
public:
  ...
  Matrix(const T (&a)[n1][n2])
      : Matrix_base<T>(n1 * n2), d1(n1), d2(n2)
...
public:
  ...
  Matrix(Row<T, 3> &a)
      : Matrix_base<T>(a.dim1() * a.dim2() * a.dim3(), a.p), d1(a.dim1()),
        d2(a.dim2()), d3(a.dim3()) {
  ...
  Matrix(const T (&a)[n1][n2][n3])
      : Matrix_base<T>(n1 * n2), d1(n1), d2(n2), d3(n3)
...
template <class F, class A> A apply(F f, A x) {
  A res(x, f);
  return xfer(res);
}
template <class F, class Arg, class A> A apply(F f, A x, Arg a) {
  A res(x, f, a);
  return xfer(res);
}
